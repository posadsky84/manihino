...
const PageCalendar = ({ season, selectDay, calendar, selectedDay, dayHistory, players }) => {
        ...
        {months.map((mon, monthIndex) => {
          return <div className="month">
            ...
            <div className="month-card">
              {[...Array(new Date(season, monthIndex, 1).getDay()).keys()].map((item) => (item > 0) &&
              ...
              {[...Array((new Date(season, monthIndex + 1, 0)).getDate()).keys()].map((item) => {
                ...
                return <div className={dayClass} onClick={() => selectDay(new Date(season, monthIndex + 1, item + 1))}
                />;
              })
              }
            </div>
